...
| huge-context sweep | haiku-scout ★❓ | Read-only; `★` haiku-scout for cheap/fast. **Escalate to codex-scout** when one big working context or an independent model helps (a coordinator judgment, not a candidate cell — scout lanes aren't a bake-off matrix). **Zero demand on execution plans** (baseline) — value is in research/planning, ad-hoc. |
...
- **R4 — verify-gate.** A unit with **no load-bearing verify command** (`verifiable=False`) is **never** routed to a write-worker or a bake-off trial — no re-check = no taster = no safe outsource (extends the §3 bake-off-window "bare unit" floor). It goes to the coordinator, or to a read-only executor when the shape is read-only. `is_bake_off_trial` is always `False` in this case. *Consequence, by design:* a read-only `❓` cell (scout shapes) never earns a command-gated trial and so won't flip through `route_pick`. That's intended here — a read-only worker's output is verified as **claims, not a diff** (§6: `re-check n/a n/a`), so its bake-off resolves through the coordinator's claim-check, a path outside this command-centric helper. Formal read-only candidate-sets / a claims-verified trial path are **out of scope** (scout lanes are single-model; see Scope Boundaries) — deferred, not overlooked.
