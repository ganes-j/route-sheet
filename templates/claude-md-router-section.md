...
- **Gate feedback → propose-confirm-dispatch** (`ROUTING_POLICY.md` §7). When a review gate produces feedback, never dispatch remediation in the same turn you propose it. Classify each item against the unit's spec: a **correction** (changes how a unit meets its existing goal/files/verify) inherits that unit's routing and rides as fix rounds; an **addition** (new deliverable/goal/surface) needs explicit acceptance and lands as a route-plan mini-pass row or a `## Follow-ons` entry — never riding an existing unit's dispatch.
