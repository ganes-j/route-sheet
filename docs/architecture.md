...
- **Runner + blinded judge** ([bin/bakeoff](../bin/bakeoff), [bin/bakeoff_grading.py](../bin/bakeoff_grading.py)) — a detached, coordinator-invoked runner replays eligible single-shot (llocal-native) units at headroom-adaptive concurrency, grades on verify commands plus a blinded pairwise judge (randomized A/B order, output-as-data framing, empty tool list, pinned judge+rubric), and appends field records. Implementation-unit replay is gated behind a harness feasibility spike.
