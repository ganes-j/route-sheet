...
## The bake-off knowledge base

The flywheel can only flip a cell it has evidence for, and the router's whole point — stop orchestration inline, delegate to a cheaper executor — means proven cells never get re-raced and the local fleet stays unmeasured. The bake-off knowledge base is the measurement layer that fills the gap. It ships empty; the thresholds are provisional (see the README maturity note).

- **Field-record ledger** ([bin/field_records.py](../bin/field_records.py)) — a machine-owned JSONL evidence store, separate from the human-readable policy cells (the repo's own "human-edited docs make lossy machine inputs" lesson). One record per raced candidate: shape, executor, kind (`real`/`replay`/`reverse-replay`), verify result, optional win margin, base commit, pinned provenance. Margin is optional by design — a trial with no incumbent produces a margin-less record.
- **Replay-bundle capture** — a completed unit's outcome line gains an optional, label-located `base:<sha>` token, and the wrap-up saves a bundle (base commit + frozen spec + verify commands + the incumbent's first-shot patch) a challenger can replay from. A structural secret scan (mirroring leak-check.sh) refuses a leaking bundle.
- **Eligibility gate** ([bin/bakeoff_eligibility.py](../bin/bakeoff_eligibility.py)) — a pure, deny-by-default function enforcing the §1 constraint layer: never-delegate, no-verify, and credential-gated units are ineligible; PII units admit local challengers only; `codex cloud exec` never enters a challenger set.
- **Runner + blinded judge** ([bin/bakeoff](../bin/bakeoff), [bin/bakeoff_grading.py](../bin/bakeoff_grading.py)) — replays eligible single-shot (llocal-native) units, grades on verify commands plus a blinded pairwise judge (randomized A/B order, output-as-data framing, empty tool list, pinned judge+rubric), and appends field records. Implementation-unit replay is gated behind a harness feasibility spike.
- **Triggering + visibility** ([bin/bakeoff-cron](../bin/bakeoff-cron), [templates/com.route-sheet.bakeoff-sweep.plist](../templates/com.route-sheet.bakeoff-sweep.plist)) — the sweep is global background work, so it runs on a launchd `StartInterval` (hourly), *not* a session hook. `bakeoff-cron` gates it: skip on the `.router-off` kill switch, defer while an interactive Claude session is active (no GPU contention). Activity surfaces passively at SessionStart — [hooks/router-session-context.py](../hooks/router-session-context.py) appends a one-line digest (replays since last, wins pending, ledger size) from the ledger + `router-bakeoff.log`, which doubles as a liveness check.
- **Challenger seeding** ([bin/challenger_seeding.py](../bin/challenger_seeding.py)) — emit-only proposals (never auto-writes) that reopen a proven cell as a `❓` when the candidate set changes: a new catalog model, a plausible-untested candidate, a materially-changed `❌`, or stale `✅` evidence. Seeds flow through route_pick's existing trial path — no new cell state.
- **Flywheel integration** — the flywheel reads the ledger alongside outcome lines, deduped on unit ref. `real`/`fallback` records are flip-eligible; `replay` records propose challenger-eligibility only, never a flip; nothing fires below the R19 record floor.
