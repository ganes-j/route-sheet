...
3. **Route each unit** — apply the policy in order:
   ...
   - **Bake-off on `❓` — don't fall back to coordinator.** If the matched cell is `❓` and the unit cleared the constraint layer + discipline floor (so it's verifiable), route it to the row's candidate executor as a **tagged exploration/bake-off trial**, not the coordinator — the re-check is the taster and 2–3 trials resolve the cell (`ROUTING_POLICY.md` §3 bake-off window; early-stop on 2 wins → `✅`). Be **cost-aware**: expensive shapes use the smallest real unit, a single A/B, or defer to a low-stakes matching unit — never force an expensive trial onto a critical path. (`❓` = run the bake-off, not play it safe.)
